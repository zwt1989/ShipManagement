...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        #region GetDataTable Or GetDataSet
    ...
    */
    ...
    public class Sql
    {
        ...
        /// <summary>
        /// 列名_asc 或 列名_ASC 或 列名_desc 或 列名_desc，如果格式不正确，将抛出异常。
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="columns"></param>
        /// <returns></returns>
        public Sql OrderByT<T>(IEnumerable<T> columns)
        {
            List<string> list = new List<string>();
            foreach (var item in columns)
            {
                string stra = item.ToString();
                if (stra.IndexOf('_') >= 0)
                {
                    stra = (stra.Remove(stra.LastIndexOf('_'), 1)).Insert(stra.LastIndexOf('_'), " ");
                }
                list.Add(stra);
            }
            return Append(new Sql("ORDER BY " + String.Join(", ", (from x in list select x).ToArray())));
        }
    ...
    namespace Internal
    {
        ...
        class PocoData
        {
            ...
            private static Func<object, object> GetConverter(IMapper mapper, PocoColumn pc, Type srcType, Type dstType)
            {
                ...
                if (pc != null && srcType == typeof(DateTime) && (dstType == typeof(DateTime) || dstType == typeof(DateTime?)))
                {
                    return delegate(object src) { return new DateTime(((DateTime)src).Ticks, DateTimeKind.Utc); };
                }
                    ...
                    else
                    {
                        return delegate(object src)
                        {
                            ...
                            catch (Exception)
                            {
                                ...
                                return Convert.ChangeType(src.ToString(), dstType, null);
                            }
